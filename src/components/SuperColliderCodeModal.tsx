...
interface SuperColliderCodeModalProps {
  isOpen: boolean;
  onClose: () => void;
  trigram: Trigram | null; // Allow null if no trigram selected
}
...
const SuperColliderCodeModal: React.FC<SuperColliderCodeModalProps> = ({ isOpen, onClose, trigram }) => {
  ...
  const copyToClipboard = async (text: string, type: 'synth' | 'pattern') => {
    if (!navigator.clipboard) {
      ...
      try {
        ...
        if (type === 'synth') setSynthCopySuccess('¡Copiado!');
        else setPatternCopySuccess('¡Copiado!');
      } catch (err) {
        if (type === 'synth') setSynthCopySuccess('Error');
        else setPatternCopySuccess('Error');
  ...
  return (
    <div 
      className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50 transition-opacity duration-300 ease-out opacity-100"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
      aria-labelledby={`modal-title-${trigram.id}`}
    >
      <div 
        className="bg-gray-800 p-6 rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative text-gray-200 transform transition-all duration-300 ease-out scale-100"
        onClick={e => e.stopPropagation()}
      >
        <button 
          onClick={onClose}
          className="absolute top-3 right-3 text-gray-400 hover:text-yellow-400 text-3xl font-bold leading-none"
        ...
        <div className="flex items-center mb-6">
          ...
          <div>
            <h2 id={`modal-title-${trigram.id}`} className="text-3xl font-bold text-sky-300">{trigram.name}</h2>
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="relative">
              <pre className="bg-gray-900 p-4 rounded-md overflow-x-auto text-sm text-sky-200 font-mono shadow-inner whitespace-pre-wrap break-all">
