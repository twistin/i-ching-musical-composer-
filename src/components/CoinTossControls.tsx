  ...
  return (
    <div className="my-8 p-6 bg-gray-800 rounded-lg shadow-lg w-full max-w-2xl mx-auto">
      ...
      <button
        onClick={onToss}
        disabled={isTossing}
        className="w-full bg-yellow-500 hover:bg-yellow-600 disabled:bg-yellow-800 disabled:cursor-not-allowed text-gray-900 font-bold py-3 px-6 rounded-lg text-lg transition duration-150 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-75"
      >
        {isTossing ? 'Lanzando Monedas...' : 'Lanzar Monedas (Generar Hexagrama)'}
      </button>
      {/* Display lines as they are tossed, but only show the full hexagram summary once 6 lines are collected */}
      {tossedLinesResult.length > 0 && tossedLinesResult.length < 6 && (
         <div className="mt-6">
          <h3 className="text-md font-semibold text-center text-sky-400 mb-1">Progreso del Lanzamiento:</h3>
